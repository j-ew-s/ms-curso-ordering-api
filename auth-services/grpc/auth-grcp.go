...
import (
	...
	authGrpc "github.com/j-ew-s/ms-curso-auth-grpc/auth-services"
...
func GetUserInfo(token string) (*authGrpc.UserInformation, error) {
	var cc *grpc.ClientConn
	connection := ":5500"
	...
	response, err := u.UserInfo(context.Background(), &message)

	if err != nil {
		fmt.Println(fmt.Sprintf("Erro na chamaga do UserInfo : %+v  -- Token : %+v", err, &message))
		return nil, err
	}
